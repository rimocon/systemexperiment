...
const float refs[5] = {0, 2000, 3000, 2000, 3000}; // 目標回転速度 [rpm]
float ref;
boolean is0rpm = false;
//I-P制御
//float U0 = 0;
...
float sum_e = 0;
float Kp = 0.00004;
float Ki = 0.00016*((float)Ts/1000.0); // サンプリング周期をあらかじめ掛けておく
float top = 625; //(3000rpm);
void initPWM1() {
  TCCR1A = 0b10100000;
...
// タイマ割り込み (MsTimer2) の ISR 40[msec] ごと
  ...
  if (iteration >= 100) {
   // 4[sec] ごとに
   // 目標値 ref の設定 ; 4[sec] ごとにrefの値を0rpm,2000rpm,3000rpm,2000rpm,3000rpmに変更
   iteration = 0;
   cnt++;
   if ( cnt >= 5){
    cnt = 0;
